...
**可执行性要求（plan 三要素）**：plan 是后续 Step 6 执行的唯一依据，需保证任意 AI 拿到它即可**逐字照做、完整落地**，无需回看 spec 或自行猜测。因此**每个步骤必须包含三要素**：

1. **具体执行步骤**：按顺序写清「对哪个文件/位置、做什么」的可操作指令；
2. **具体代码 / 生成内容**：给出可直接照抄的代码片段或要生成的具体文本；
3. **涉及文件范围**：列出本步改动的确定文件路径（标注新增/修改/删除）。

生成时禁止模糊表述（如「实现登录功能」「完善样式」「补充相应逻辑」这类留给执行者自行发挥的措辞）；拿不准的取舍在 plan 里直接写死，执行阶段不做二次决策。plan.md 采用如下结构：
...
- 目标：（本步达成什么）
- 涉及文件：
  - `{相对路径}`（{新增/修改/删除}）：{本步对它的改动说明}
- 具体执行步骤：
  1. {动作 1，含目标文件/位置}
  2. {动作 2}
  ...
- 具体代码 / 生成内容：
  （可直接照抄的代码块，含函数签名、关键逻辑、异常与边界处理；
   生成文案/配置/文档则写出要写入的具体文本；
   本步无内容可写时注明"无"）
- 验收要点：（如何判断本步完成，如运行某命令、检查某文件包含某内容）
...
- **具体执行步骤**：写成可操作指令并按执行顺序编号；涉及多文件时逐条列出，不得用「等」「等等」「类似」收尾概括。
- **具体代码 / 生成内容**：改动代码给出完整可复制的代码块（含函数/类签名、关键逻辑、异常与边界处理）；生成文案、配置、文档等给出要写入的具体文本，新增文件的整体内容可直接展开给出。
- **涉及文件范围**：必须写确定相对路径并标注「新增 / 修改 / 删除」；各步骤的文件清单与开头「文件范围总览」保持一致。

> 轻量约定：每步可独立完成即可，不汇总 allowedPaths，不启动 plan-reviewer。

### Step 5：plan 门禁（用户选择）

向用户展示 plan 概要（重点确认每步是否已具备三要素：具体执行步骤 / 具体代码或生成内容 / 涉及文件范围；若某步仍不够具体，提示用户可通过选项 2/3 要求重写或补充细节），并给出三个选项：
...
严格按 `plan.md` 从步骤 1 依次执行，不做 plan 未写明的自行发挥；若发现 plan 某处有误或信息不足以执行，停下来向用户说明并由用户决定（修正 plan 后继续）：

1. 每步完成后可运行必要的验证/测试（轻量原则：不强制分层验证），验收以 plan 中「验收要点」为准。
...
| 10 | 生成 plan 时每步必须含三要素：**具体执行步骤 / 具体代码或生成内容 / 涉及文件范围**，保证后续 AI 可逐字照做完整执行 |
