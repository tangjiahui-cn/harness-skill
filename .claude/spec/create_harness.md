...
description: 完整的软件开发工作流 skill。用户输入文本需求后，自动生成规格文档(spec)、生成执行计划(plan)、逐步执行计划(每步git commit)、运行测试，通过后询问是否推送到远程。使用场景：任何需要从零开始实现一个功能/模块的场景，特别是需要结构化追踪开发过程时。本 skill 遵循标准 skill 仓库格式，位于 skills/spec-create-harness/。
---
...
| `.claude/spec/spec_create_harness.md` | 本 skill 的规格文档（即当前文件），用于指导 skill 的开发和评审 |
...
```json
{
  ...
  "step": 2,
  "status": "initialized",
  "createdAt": "2026-07-27T10:00:00.000Z",  // 实际使用时替换为当前时间
  "updatedAt": "2026-07-27T10:00:00.000Z",  // 初始状态与 createdAt 相同
...
spec 生成阶段由**两个独立子 Agent** 协作完成（注意：每次循环都启动新的 Agent 实例，角色名仅描述职责）：
...
父 Agent 启动 **spec-generator Agent**，分配以下任务：

**spec-generator Agent 的职责：**
...
| **spec-generator Agent（修补模式）** | 读取 `spec-suggest.md`，逐条评估并采纳合理建议，修改 `spec.md`。不质疑 reviewer 的发现，专注修补 |
| **父 Agent** | 读取 `spec-suggest.md` 判断是否有实质性问题；驱动循环流程；管理临时文件 |
...
plan 生成阶段与 spec 生成阶段采用相同的**双 Agent 协作模式**（同样每次启动新的 Agent 实例）：
...
父 Agent 启动 **plan-generator Agent**，分配以下任务：

**plan-generator Agent 的职责：**
...
| **plan-generator Agent（修补模式）** | 读取 `plan-suggest.md`，逐条评估并采纳合理建议，修改 `plan.md` |
| **父 Agent** | 读取 `plan-suggest.md` 判断是否有实质性问题；驱动循环流程；管理临时文件 |
...
1. **写入 allowedPaths**：将 plan-generator Agent（最新修补后）报告的 `allowedPaths` 写入 `state.json`：
   ```json
   {
...
- **1 推送** → 执行 `git push`
  ...
  - ❌ push 失败 → 分析失败原因并提供对应处理：
    ...
    - **权限不足/网络问题** → 重试或手动推送
    - **远程冲突（非快进）** → 提供选项：
      > 1️⃣ 强制推送（`git push --force`，适用于功能分支，⚠️ 会覆盖远程历史）
      > 2️⃣ 先 pull 再推送（`git pull --rebase && git push`）
      > 3️⃣ 手动处理
      > 4️⃣ 跳过推送（代码保留在本地）
...
- **选项编号统一**：所有用户选择界面使用 `1️⃣ 2️⃣ 3️⃣ 4️⃣ 5️⃣` 数字序号格式，保持一致性
- **确认节点**：spec 确认、plan 确认、推送确认三个节点必须等用户明确回复
- **错误透明**：出错了告知用户错误信息和你的分析，不要默默重试
- **适度灵活**：用户可能在过程中提出修改需求，回到对应环节重新调整
- **Agent 命名**：各阶段 Agent 职责不同，使用 `spec-generator` / `spec-reviewer` / `plan-generator` / `plan-reviewer` / `execution` 区分，每次循环启动新实例
- **子 Agent 协作**：spec 和 plan 的生成均交给独立子 Agent 处理，父 Agent 只负责启动子 Agent、接收报告、与用户交互确认。子 Agent 与父 Agent 通过校验建议文件（`spec-suggest.md` / `plan-suggest.md`）协作——reviewer 输出结构化建议，父 Agent 读取并判断是否有实质性问题，然后驱动下一轮生成或退出。子 Agent 之间不直接交互，所有通信通过父 Agent 协调
...
2. 如果 `.harness/temp/` 中**存在** `{vId}` 子目录，且 `state.json` 的 `status` 不是 `"accepted"` 或 `"completed"`：
   ...
   - **多个未完成** → 列出所有未完成的 `{vId}`（含需求名、状态、上次更新时间），让用户选择恢复其中一个（**其他未完成需求保留在 `.harness/temp/` 中，可下次恢复**），或选择"开始新的需求"：
     ...
     > 检测到多个未完成的需求：
     >
     > 1️⃣ 登录功能 (登录功能_aBcDeFgHiJ) — executing, 上次更新 2026-07-27T10:00
     > 2️⃣ 注册功能 (注册功能_zYxWvUtSrQ) — spec_reviewing, 上次更新 2026-07-26T15:00
     > 3️⃣ 开始新的需求（丢弃以上所有）
     ```
     用户选择恢复 1️⃣ 或 2️⃣ 后，仅恢复选中的需求，其他未完成需求保留在 `.harness/temp/` 中。
   ```
   > 检测到未完成的需求：
   > - 需求：{name}
   > - ID：{vId}
